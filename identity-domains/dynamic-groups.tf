...
data "oci_identity_domain" "dyngrp_domain" {
  ...
  domain_id = each.value.identity_domain_id != null ? length(regexall("^ocid1.*$", each.value.identity_domain_id)) > 0 ? each.value.identity_domain_id : (contains(keys(var.identity_domains_dependency), each.value.identity_domain_id) ? var.identity_domains_dependency[each.value.identity_domain_id].id : "__VOID__") : length(regexall("^ocid1.*$", var.identity_domain_dynamic_groups_configuration.default_identity_domain_id)) > 0 ? var.identity_domain_dynamic_groups_configuration.default_identity_domain_id : (contains(keys(var.identity_domains_dependency), var.identity_domain_dynamic_groups_configuration.default_identity_domain_id) ? var.identity_domains_dependency[var.identity_domain_dynamic_groups_configuration.default_identity_domain_id].id : "__VOID__")
}
