...
locals {
  ...
  application_types         = ["SAML", "Mobile", "Confidential", "Enterprise","SCIM","FusionApps"]
  ...
  authn_server_op           =  { for k,v in var.identity_domain_applications_configuration != null ? var.identity_domain_applications_configuration.applications : {} :  k => 
                 ...
                 if v.type == "SCIM" && v.enable_provisioning == true
                 }
  ...
  provisioning_app_types =  ["SCIM","FusionApps"]
...
resource "oci_identity_domains_app" "these" {
  for_each       = var.identity_domain_applications_configuration != null ? var.identity_domain_applications_configuration.applications : {}
    lifecycle {
      ...
      precondition {
        condition = each.value.redirect_urls == null && !contains(["SAML","SCIM","FusionApps"],each.value.type) ? !(contains(local.grant_types, "implicit")||contains(local.grant_types, "authorization_code"))  : true
    ...
    based_on_template {
            value = each.value.type == "Confidential" ? "CustomWebAppTemplateId" : (each.value.type == "SAML" ? "CustomSAMLAppTemplateId" : (each.value.type == "Enterprise" ? "CustomEnterpriseAppTemplateId" : (each.value.type == "Mobile" ? "CustomBrowserMobileTemplateId" : (each.value.type == "SCIM" ? "170c671e7cc13a78a480a2d1f8a5d123" : (each.value.type == "FusionApps" ? "417374eb54633da0929a71a5545ebf4c" : null))))) # "170c671e7cc13a78a480a2d1f8a5d123" : null))))
    }
