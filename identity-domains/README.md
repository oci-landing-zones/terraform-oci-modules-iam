...
- **ignore_external_membership_updates**: (Optional) defines whether this module does not overwrite group members after group creation. Default is true, meaning members added to the group via some other (external) means (OCI Console, API, SCIM, JIT, etc) are preserved on any subsequent executions of this module (the attribute name implies in ignoring any external membership updates, by not changing them). Set this to false for resetting group members per the inputs provided to the module. **This attribute is to be used as a one time setting. Changing it later on will trigger recreation of all groups given in *groups* input attribute.** 
- **default_defined_tags**: (Optional) defined tags to apply to all resources, unless overridden by *defined_tags* attribute within each resource.
- **default_freeform_tags**: (Optional) freeform tags to apply to all resources, unless overridden by *freeform_tags* attribute within each resource.
